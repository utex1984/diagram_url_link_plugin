...
function status_apply(_selection: readonly SceneNode[], enabled: boolean): string {
...
function evaluateEnabled_read(selection: readonly SceneNode[]): boolean {
  return selection.length === 1
}
function actionTarget_read(): SceneNode | null {
  const target = singleSelectedTarget()
  if (target == null) return null
...
async function action_read(_params: Params, target: SceneNode, _previousState: unknown | null): Promise<{ affectedNodes: SceneNode[]; state: unknown | null }> {
  const affectedNodes: SceneNode[] = [target]
  ;(() => {
...
async function runAction_read(target: SceneNode, notify: boolean): Promise<void> {
  isExecuting = true
  try {
